...

{
  ...
  const fetchAssistantResponse = async () => {
    ...
    const siteMapUrl = `https://insiderperks.com/wp-content/endpoints/campspot/sitemaps/${pID}.png`;
  ...
  return res.status(200).json({ summarizedCamps: response2, availableCamps: toolResponseObj, urlEndpoint: urlWithParams });
}
